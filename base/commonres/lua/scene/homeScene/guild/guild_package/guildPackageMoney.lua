...
guildPackageMoney = class("guildPackageMoney", MGLayer)

function guildPackageMoney:ctor()
    self.goodsNum = 1;
    self.index = 0;
    self.maxNum = 1;
    self.tag = 1;
    self.curGm = nil;
    self.day_money_exp = 0;
    self.my_feats = 0;
    self:init();
end

function guildPackageMoney:init()
    local pWidget = MGRCManager:widgetFromJsonFile("guildPackageMoney","guild_package_donation_money_ui.ExportJson");
    ...
    Label_kindNmae:setText(MG_TEXT_COCOS("guild_package_donation_money_ui_1"));

    local Label_numName = self.Panel_3:getChildByName("Label_numName");
    Label_numName:setText(MG_TEXT_COCOS("guild_package_donation_money_ui_2"));

    local Label_apply1 = self.Panel_use:getChildByName("Label_apply1");
    Label_apply1:setText(MG_TEXT_COCOS("guild_package_donation_money_ui_3"));

    local Label_apply2 = self.Panel_use:getChildByName("Label_apply2");
    Label_apply2:setText(MG_TEXT_COCOS("guild_package_donation_money_ui_4"));

    local Label_btn = self.Button_apply:getChildByName("Label_btn");
    Label_btn:setText(MG_TEXT_COCOS("guild_package_donation_money_ui_5"));

    local sql = string.format("select value from config where id=114");
    local DBData = LUADB.select(sql, "value");
    self.value = tonumber(DBData.info.value);
    Label_tip:setText(string.format(MG_TEXT("guildPackageLayer_6"),self.value));

    self:setData();
end

function guildPackageMoney:setData()
    ...
    if self.gmList then
        ...
        for i=1,#self.gmList do
            if self.gmList[i]:unionMoneyExp() > 0 then--可捐献资金
...
function guildPackageMoney:createListItem()
...
function guildPackageMoney:createItem(gm)
    if gm then
        self.curGm = gm;
        ...
        self.Label_num2:setText(self.curGm:unionMoney()*self.goodsNum);
        self.Label_num3:setText(string.format("%d/%d",self.goodsNum*gm:unionMoneyExp()+self.day_money_exp,self.value));
...
function guildPackageMoney:ItemSelect(item)
...
function guildPackageMoney:setGoodsNum()
    self.goodsNum = self.goodsNum + self.index;
    if self.curGm:unionMoneyExp()*self.goodsNum + self.day_money_exp > self.value then
    ...
    self.Label_num1:setText(self.goodsNum);
    self.Label_num2:setText(self.curGm:unionMoney()*self.goodsNum);
    self.Label_num3:setText(string.format("%d/%d",self.curGm:unionMoneyExp()*self.goodsNum + self.day_money_exp,self.value));
end

function guildPackageMoney:onButtonClick(sender, eventType)
    ...
    elseif eventType == ccui.TouchEventType.ended then
        ...
        elseif sender == self.Button_max then--最大
            if self.maxNum*self.curGm:unionMoneyExp()+self.day_money_exp < self.value then
                self.goodsNum = self.maxNum;
            else
                self.goodsNum = math.floor((self.value-self.day_money_exp)/self.curGm:unionMoneyExp());
            end
            self.Label_num1:setText(self.goodsNum);
            self.Label_num2:setText(self.curGm:unionMoney()*self.goodsNum);
            self.Label_num3:setText(string.format("%d/%d",self.curGm:unionMoneyExp()*self.goodsNum+self.day_money_exp,self.value));
...
function guildPackageMoney:onReciveData(MsgID, NetData)
    print("guildPackageMoney onReciveData MsgID:"..MsgID)

    if MsgID == Post_donateMoney then
...
function guildPackageMoney:sendReq()
    local str = string.format("&id=%s&num=%d",self.curGm:getId(),self.goodsNum);
    NetHandler:sendData(Post_donateMoney, str);
end

function guildPackageMoney:pushAck()
    NetHandler:addAckCode(self,Post_donateMoney);
    
end

function guildPackageMoney:popAck()
    NetHandler:delAckCode(self,Post_donateMoney);
end

function guildPackageMoney:onEnter()
    self:pushAck();
end

function guildPackageMoney:onExit()
    self:popAck();
    MGRCManager:releaseResources("guildPackageMoney");
    if self.schedulerID then
        cc.Director:getInstance():getScheduler():unscheduleScriptEntry(self.schedulerID);
    end
end

function guildPackageMoney.create(delegate)
    local layer = guildPackageMoney:new()
